...
st.title("Professor-Course Assignment Network Flow Diagram (Adjusted Edge Labels)")
...
# Draw edge labels offset slightly to avoid overlap
edge_labels = {(u, v): f"{d['capacity']}, {d['weight']}" for u, v, d in G.edges(data=True)}
label_pos = {edge: ((pos[edge[0]][0] + pos[edge[1]][0]) / 2,
                    (pos[edge[0]][1] + pos[edge[1]][1]) / 2 + 0.2)
             for edge in edge_labels.keys()}  # offset Y by +0.2

nx.draw_networkx_edge_labels(G, label_pos, edge_labels=edge_labels, font_size=9)
